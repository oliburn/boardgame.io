 ...
 */
const CreateMatch = async ({ res, db, uuid, ...opts }) => {
    const matchID = uuid();
...
const configureRouter = ({ router, db, auth, games, uuid = () => nanoid.nanoid(11), }) => {
     ...
     */
    router.post('/games/:name/create', express__default["default"].json(), async (req, res) => {
        ...
        if (!game)
            return res.status(404).send('Game ' + gameName + ' not found');
        if (req.body.numPlayers !== undefined &&
    ...
    router.get('/games/:name', async (req, res) => {
        const gameName = req.params.name;
        const isGameoverString = unwrapQuery(req.query.isGameover);
        const updatedBeforeString = unwrapQuery(req.query.updatedBefore);
        const updatedAfterString = unwrapQuery(req.query.updatedAfter);
        let isGameover;
        if (isGameoverString) {
     ...
     */
    router.get('/games/:name/:id', async (req, res) => {
     ...
     */
    router.post('/games/:name/:id/join', express__default["default"].json(), async (req, res) => {
        ...
        if (typeof playerID === 'undefined' || playerID === null) {
            ...
            if (playerID === undefined) {
                ...
                return res
                    .status(409)
                    .send(`Match ${matchID} reached maximum number of players (${numPlayers})`);
            }
        }
     ...
     * @return - Nothing.
     */
    router.post('/games/:name/:id/leave', express__default["default"].json(), async (req, res) => {
        const matchID = req.params.id;
     ...
     */
    router.post('/games/:name/:id/playAgain', express__default["default"].json(), async (req, res) => {
        const gameName = req.params.name;
        const matchID = req.params.id;
        const playerID = req.body.playerID;
        const credentials = req.body.credentials;
    ...
    const updatePlayerMetadata = async (req, res) => {
        const matchID = req.params.id;
        const playerID = req.body.playerID;
        const credentials = req.body.credentials;
     ...
     * @return - Nothing.
     */
    router.post('/games/:name/:id/rename', express__default["default"].json(), async (req, res) => {
     ...
     * @return - Nothing.
     */
    router.post('/games/:name/:id/update', express__default["default"].json(), updatePlayerMetadata);
    return router;
};
const configureApp = (app, router, origins) => {
    ...
    //   },
    // })
    cors__default["default"]({ origin: origins }));
...
class SocketIO {
    ...
    init(app, games, origins = []) {
        ...
        this.io = new socket_io.Server(app.server, {
        ...
        for (const game of games) {
            ...
            nsp.on('connection', (socket) => {
                socket.on('update', async (...args) => {
                    ...
                    const master = new Master(game, app.db, TransportAPI(matchID, socket, filterPlayerView, this.pubSub), app.auth);
    ...
    return {
        ...
        run: async (portOrConfig, callback) => {
            ...
            if (!lobbyConfig || !lobbyConfig.apiPort) {
                ...
                const api = express__default["default"]();
            ...
            info(`App serving on ${getPortFromServer(app.server)}...`);
